(() => {
  ...
  let mounting = false;
  const FONTS_JSON = "/tanba/data/fonts.json?v=tanba14";
  ...
  function decorateCatalog(cards) {
    ...
    cards.forEach((card) => {
      ...
      if (h3 && card.dataset.slug && !h3.querySelector("a")) {
        ...
        link.href = `/tanba/font/${card.dataset.slug}/`;
  ...
  async function loadFontIndex() {
    ...
    try {
      ...
      try {
        const res = await fetch("/tanba/data/fonts.json");
